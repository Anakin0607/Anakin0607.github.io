...
如果No working init found，也说明内核成功启动，因为init进程是内核启动后第一个启动的进程，其他所有的进程都是init进程的子进程。init进程一般位于rootfs目录中，下面的章节将开始构建rootfs

### 构建rootfs
rootfs利用busybox工具构建，首先下载busybox源码包并解压
...
### 构建引导
树莓派的启动流程为：引导程序->内核->rootfs中的代码，其中引导程序和硬件绑定，内核会自动选择rootfs中的程序，因此我们只需要让引导程序装入我们自己构建的内核，并正确执行，即可使树莓派正确运行。因此这一步选择用树莓派官方提供的引导程序，来构建整个引导目录boot/
